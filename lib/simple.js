...
export function protoType() {
    ...
    Number.prototype.toTimeString = function toTimeString() {
        ...
        return (
(days ? `${days} ${langTOGGLE.smsAfkResultTime[0]} ` : '') + (hours ? `${hours} ${langTOGGLE.smsAfkResultTime[1]} ` : '') +
(minutes ? `${minutes} ${langTOGGLE.smsAfkResultTime[2]} ` : '') + (seconds ? `${seconds} ${langTOGGLE.smsAfkResultTime[3]}` : '')).trim()
    }
